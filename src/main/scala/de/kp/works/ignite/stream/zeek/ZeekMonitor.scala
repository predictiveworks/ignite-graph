...
class ZeekMonitor(zeekFolder: String, eventHandler: ZeekEventHandler) {

  import actor.FileActor._
  /**
   ...
   * use cases somewhat.
   */
  private val systemName = WorksConf.getSystemName(WorksConf.ZEEK_CONF)
  ...
  def start(): Unit = {
     ...
     * to listen to changes in the Zeek log folder. This source will emit elements
     * every time there is a change to a watched directory in the local filesystem,
     */
    val changes = DirectoryChangesSource(
      fs.getPath(zeekFolder), pollInterval = pollingInterval, maxBufferSize = maxBufferSize)
    /*
     * This is the entry point for the Zeek log file monitor
     *
     * We expect that the Zeek log directory does not contain
  ...
  private def isLogFile(path:Path):Boolean = {
     ...
     * sub directories within the Zeek log folder
     */
   ...
   */
  private def buildFileActor(path:Path):ActorRef = {
    val actorName = "File-Actor-" + java.util.UUID.randomUUID.toString
    system.actorOf(Props(new FileActor(path, eventHandler)), actorName)
  }
}
